...
describes what users get. Pick a format with `-o`, or let exercism choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
exercism <command> -o table   # aligned columns for reading
exercism <command> -o jsonl   # one JSON object per line, for piping
exercism <command> -o json    # a single JSON array
exercism <command> -o csv     # spreadsheet friendly
exercism <command> -o tsv     # tab-separated
exercism <command> -o url     # just the URL column
exercism <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
exercism <command> --fields id,title,url
```
...
```bash
exercism <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
exercism <command>            # a table, because this is a terminal
exercism <command> | wc -l    # JSONL, because this is a pipe
```
