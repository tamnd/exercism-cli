// Package cli builds the exercism command tree on top of the exercism library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: exercism.DefaultConfig()}

	root := &cobra.Command{
		Use:   "exercism",
		Short: "Browse programming exercises on exercism.org",
		Long: `exercism reads exercism.org through its public API (no key required).

It lists programming tracks, exercises, and concepts, and returns records as
table, JSON, JSONL, CSV, TSV, or URLs.

exercism is an independent tool and is not affiliated with Exercism.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.StringVar(&app.cfg.BaseURL, "base-url", app.cfg.BaseURL, "override base URL")
...
func (a *App) setup() error {
	...
	a.client = exercism.NewClient(a.cfg)
	return nil
}
